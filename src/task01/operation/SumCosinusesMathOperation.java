...
public class SumCosinusesMathOperation implements MathOperation
{
    ...
    public Number calculate(ArrayList<Number> numbers, int type)
    {
        ...
        if (type == 1)
        {
            ...
            {
                sum = sum.floatValue() + Math.cos(Math.toRadians(number.floatValue()));
            }
        } else if (type == 2)
        {
            for (Number number : numbers)
            {
                sum = sum.floatValue() + Math.cos(number.floatValue());
            }
        }
        return sum;
    }
